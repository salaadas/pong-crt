...
Dynamic_Font *get_font_at_size(String font_folder, String name, i32 pixel_height)
{
    ...
    // printf("Making a new font '%s' at height %d\n", temp_c_string(name), pixel_height);
...
void generate_quads_for_prepared_text(Dynamic_Font *font, i64 x, i64 y)
{
    ...
    array_free(&font->current_quads); // @Speed: Consider using the temporary storage after we fix the issue.
...
void deinit_all_font_stuff_on_resize()
{
    ...
    {
        assert(it->texture.id != 0);
        glDeleteTextures(1, &it->texture.id);
