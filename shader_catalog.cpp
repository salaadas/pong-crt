...
bool load_shader_from_memory(String shader_text, Shader *shader)
{
    ...
    auto c_v = (char*)to_c_string(vert_string);
    auto c_f = (char*)to_c_string(frag_string);
    ...
    if (shader->program)
    {
        // Delete the old shader program if we are reloading the shader.
        glDeleteProgram(shader->program);
        shader->dirty = true; // Mark is as dirty in order to call glUseProgram again in set_shader().
    }
...
void reload_asset(Shader_Catalog *catalog, Shader *shader)
{
    ...
    // defer {free_string(&original_data);}; @Leak:
    ...
    if (!shader->is_header)
    {
        ...
        if (!success)
        {
            logprint("shader_catalog", "Shader '%s' loaded but failed to compile.\n", temp_c_string(shader->name));
            return;
        }
    }
