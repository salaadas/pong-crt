...
void DumpShaderInfoLog(GLuint shader, String name)
{
    ...
    if (length_result > 0)
    {
        ...
        s.data  = buffer.data;
        logprint("ShaderInfoLog", "In shader '%s':\n%s\n", temp_c_string(name), temp_c_string(s));
    }
}
...
void DumpProgramInfoLog(GLuint program, String name)
{
    ...
    if (length_result > 0)
    {
        ...
        s.count = length_result;
        s.data  = buffer.data;
        logprint("ProgramInfoLog", "In shader '%s':\n%s\n", temp_c_string(name), temp_c_string(s));
    }
}
...
void immediate_flush()
{
    ...
    if (true) // if (!vertex_format_set_to_XCNUU)
    {
        ...
        // @Incomplete: Fix this!!!!
        // @Incomplete: Fix this!!!!
        // I think this is interfering with us setting the same diffuse texture twice in a row.
...
my_pair<Texture_Map*, Texture_Map*> create_texture_render_target(i32 width, i32 height, bool do_depth_target, bool do_hdr, bool do_multisampling)
{
